...
create policy conversations_participant_insert on public.conversations for insert with check (true);
create policy participants_self_insert on public.conversation_participants for insert with check (profile_id = public.current_profile_id() or exists (select 1 from public.conversations c where c.id = conversation_id));
